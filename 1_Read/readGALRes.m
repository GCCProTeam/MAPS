function [resE1, resE5a] = readGALRes(fid, filename)
%Reading Galileo residual datas
...
%OUTPUT:
%resE1；cell array containing E1 residual data
%resE5a；cell array containing E5a residual data
...
max_rows = 86400;
resE1 = cell(max_rows, 1);
resE5a = cell(max_rows, 1);
...
for i = 1:length(lines)
    ...
    if startsWith(line, '$GPST')
     ...
     %Check if the line starts with 'E' (satellite data)      
    elseif startsWith(line, 'E')
        ...
        %Check if the line contains E1 and E5a data
        if numel(data) >= 11
           valid1 = str2double(data{4});%satellite valid flag of E1
           ...
           %Store E1 and E5a data
           i_1 = i_1 + 1;
           i_2 = i_2 + 1;
           resE1{count}(i_1, :) = [timeWeek, timeSow, satNum, valid1, az,...
               el, resc1, resp1, cn01, stat];
           resE5a{count}(i_2, :) = [timeWeek, timeSow, satNum, valid2, az,...
               el, resc2, resp2, cn02, stat];
         
           %Check if the line contains only E1 data
...
resE1 = resE1(~cellfun(@isempty, resE1));
resE5a = resE5a(~cellfun(@isempty, resE5a));
fclose(fid);
disp(['Read successfully: ', filename,' GAL']);
end
