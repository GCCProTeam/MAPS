function [std, mfw, ztd] = tropoHopfield(time, el, met, est_ZWD)
...
%time: in sec of week
%el: elevation [rad]
%met: pressure [mbar] temperature[¡ãC]  relative humidity [%]
...
if numel(met) == 3      % values from GUI
    T = met(1,1);       % temperature [¡ãC]
    p = met(2,1);       % pressure of air [mbar]
    rh = met(3,1);      % relative humidity [%]
    e = calPartialPressureWaterVapor(T, rh);	% partial pressure of water vapor [hPa]
else    
    ...
    e = calPartialPressureWaterVapor(T, rh);
end

T = T + 273.16;      % Temperature in Kelvin [K]
